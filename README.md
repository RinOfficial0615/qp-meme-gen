...
默认在选框内以中心竖线为轴做水平翻转，也可切换为中心横线的垂直翻转。水平翻转处理左右对称，垂直翻转处理上下对称。输出尺寸与输入相同，框外像素不动。叠加文字先画到源图，再走同一套镜像。

## 功能

- 打开图片：按钮、拖入窗口、拖到 exe、Ctrl+V / Shift+Insert 粘贴
- 默认框选：单人脸 / 多人脸 / 整张图片
- 多处框选：Ctrl 多选、Ctrl+A 全选、整体移动；每框独立镜像方式、保留侧、查看原图和角标
- 镜像方式：水平翻转 / 垂直翻转；保留侧支持自动及对应的左 / 右或上 / 下
- 编辑历史：Ctrl+Z 撤销、Ctrl+Y 或 Ctrl+Shift+Z 重做；工具栏 ↶ / ↷ 按钮显示可用状态
...
3. 普通点击单选，Ctrl+点击可多选；工具栏显示 `已选/总数`。选中框即使关闭角标，也会保留独立的角落勾选标记。
4. 需要字幕时点「文字」，再点画面输入；文字会跟选框一起按当前镜像方式对称。
5. 需要回退时按 Ctrl+Z；按 Ctrl+Y（或 Ctrl+Shift+Z）重做。**保存图片** 或 **复制图片**。
...
default_mirror_axis = "horizontal" # horizontal = 左右，vertical = 上下
default_keep_side = "auto"      # "auto" | "left" | "right" | "top" | "bottom"
