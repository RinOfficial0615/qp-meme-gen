//! 镜像核心算法：纯函数，无 GUI 依赖。
//!
//! 语义：输出尺寸与输入一致。选框内以"框中心竖线"为轴，把保留侧的一半
//! 水平翻转后覆盖另一侧；选框外像素保持不变。
...
/// 自动方向：选"素材侧信息量更高"的一侧保留（梗图笑点来自被复制的半张脸，
/// 信息多的一侧复制过去效果更明显）。信息侧重度 = 保留半区的水平梯度能量
/// （相邻列亮度差平方和），隔行采样控制大图开销；打平取左。
pub fn auto_direction(img: &RgbaImage, sel: Rect) -> Direction {
    let l = side_energy(img, sel, Direction::Left);
    let r = side_energy(img, sel, Direction::Right);
    if l >= r {
        Direction::Left
    } else {
        Direction::Right
...
/// 指定保留侧（框内轴以左/以右）的水平梯度能量。
fn side_energy(img: &RgbaImage, sel: Rect, dir: Direction) -> f64 {
    let (w, h) = (img.width() as i32, img.height() as i32);
    let sel = sel.normalized().clamped(w, h);
    if !sel.is_mirrorable() {
        return 0.0;
    }
    let axis2 = sel.x0 + sel.x1;
    let axis = axis2 / 2; // floor(axis)，左右分界参照
    let (xa, xb) = match dir {
        Direction::Left => (sel.x0, axis),
        Direction::Right => (axis, sel.x1 - 1),
    };
    if xb <= xa {
        return 0.0;
    }
